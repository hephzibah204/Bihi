...
const ModernReportCard = ({ student, students, scores, subjects, settings, term, session, remarks, attendance }) => {
    ...
    return (
        <div className="bg-white p-10 font-sans" style={{ width: '210mm', minHeight: '297mm' }}>
            <header className="flex justify-between items-center pb-6 border-b-2 border-indigo-500">
                ...
                <img src={settings.schoolLogo} alt="Logo" className="w-16 h-16 rounded-full"/>
            </header>
            ...
            <section className="grid grid-cols-2 gap-12">
                <div>
                    ...
                    <div className="space-y-2">
                        {results.map(res => (
                            <div key={res.subjectName} className="flex justify-between items-center text-sm p-2 rounded-md hover:bg-gray-50">
                ...
                <div>
                    <h3 className="font-semibold mb-3">Performance Summary</h3>
                     <div className="bg-indigo-50 p-4 rounded-lg space-y-3">
                        <div className="flex justify-between text-sm"><span>Average Score</span> <span className="font-bold">{performance.average}%</span></div>
                        <div className="flex justify-between text-sm"><span>Class Position</span> <span className="font-bold">{performance.position} / {performance.totalStudentsInClass}</span></div>
                    </div>
