...
const TopBarTeacher: React.FC<{ pageTitle: string }> = ({ pageTitle }) => {
    ...
    <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6">
      ...
      <div className="flex items-center gap-3">
        <div className="hidden md:flex items-center gap-2 border rounded-full px-3 py-1.5">
          ...
          <input className="outline-none text-sm placeholder:text-gray-400" placeholder="Search students, classes…" value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{ if (e.key==='Enter') navigateView(TEACHER_VIEWS.MY_STUDENTS as any); }} />
        </div>
        ...
        <IconBtn title="Help" onClick={() => navigateView(TEACHER_VIEWS.HELP as any)}><QuestionMarkCircleIcon className="w-5 h-5" /></IconBtn>
        <IconBtn title="Messages" onClick={() => navigateView(TEACHER_VIEWS.MESSAGES as any)}><ChatBubbleLeftRightIcon className="w-5 h-5" /></IconBtn>
        <IconBtn title="Notifications" onClick={() => navigateView(TEACHER_VIEWS.NOTIFICATIONS as any)}><BellIcon className="w-5 h-5" /></IconBtn>
        <IconBtn title="Settings" onClick={() => navigateView(TEACHER_VIEWS.SETTINGS as any)}><Cog6ToothIcon className="w-5 h-5" /></IconBtn>
        <div className="relative flex items-center gap-2 ml-2">
          ...
          {menuOpen && (
            <div className="absolute right-0 top-10 bg-white border border-gray-200 rounded-xl shadow p-2 w-40">
              <button className="w-full text-left px-3 py-2 rounded hover:bg-gray-50 text-sm" onClick={() => { navigateView(TEACHER_VIEWS.PROFILE as any); setMenuOpen(false); }}>Profile</button>
              <button className="w-full text-left px-3 py-2 rounded hover:bg-gray-50 text-sm" onClick={() => { navigateView(TEACHER_VIEWS.SETTINGS as any); setMenuOpen(false); }}>Settings</button>
