...
export async function requirePlatformRoles(request, env, roles = []) {
  try {
    const authHeader = request.headers.get('Authorization');
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return { ok: false, res: new Response(JSON.stringify({ error: 'Unauthorized: Missing token' }), { status: 401 }) };
    }
    const token = authHeader.split(' ')[1];
    const { SUPABASE_URL, SUPABASE_ANON_KEY } = env || {};
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
      return { ok: false, res: new Response(JSON.stringify({ error: 'Server not configured' }), { status: 500 }) };
    }
    const ures = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
      headers: { 'Authorization': `Bearer ${token}`, 'apikey': SUPABASE_ANON_KEY }
    });
