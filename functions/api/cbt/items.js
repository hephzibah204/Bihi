...
export async function onRequest(context) {
  ...
  try {
    ...
    if (request.method === 'GET') {
      ...
      if (ids) q.searchParams.set('id', `in.(${ids})`);
      if (type) q.searchParams.set('type', `eq.${type}`);
      if (tags) q.searchParams.set('tags', `cs.{${tags}}`);
      if (lo) q.searchParams.set('learning_objectives', `cs.{${lo}}`);
      if (dmin) q.searchParams.set('difficulty', `gte.${dmin}`);
      if (dmax) q.searchParams.set('difficulty', `lte.${dmax}`);
