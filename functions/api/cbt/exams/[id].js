...
export async function onRequest(context) {
  ...
  try {
    ...
    if (request.method === 'PUT') {
      ...
      const r = await fetch(`${env.SUPABASE_URL}/rest/v1/cbt_exams?id=eq.${id}`, { method: 'PATCH', headers, body: JSON.stringify(body) });
      if (!r.ok) {
    ...
    if (request.method === 'DELETE') {
      const r = await fetch(`${env.SUPABASE_URL}/rest/v1/cbt_exams?id=eq.${id}`, { method: 'DELETE', headers });
      if (!r.ok) {
