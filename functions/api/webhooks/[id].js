...
export async function onRequest(context) {
  ...
  try {
    ...
    if (request.method === 'PATCH') {
      ...
      if (body?.status && (body.status === 'active' || body.status === 'inactive')) patch['status'] = body.status;
      if (Object.keys(patch).length === 0) {
        res = new Response(JSON.stringify({ error: 'Nothing to update' }), { status: 400 });
      } else {
        const r = await fetch(`${env.SUPABASE_URL}/rest/v1/webhooks?id=eq.${id}`, { method: 'PATCH', headers, body: JSON.stringify(patch) });
        if (!r.ok) throw new Error(await r.text());
        res = new Response(JSON.stringify({ ok: true }), { status: 200 });
      }
    } else if (request.method === 'DELETE') {
      const r = await fetch(`${env.SUPABASE_URL}/rest/v1/webhooks?id=eq.${id}`, { method: 'DELETE', headers });
      if (!r.ok) throw new Error(await r.text());
      res = new Response(JSON.stringify({ ok: true }), { status: 200 });
    } else {
